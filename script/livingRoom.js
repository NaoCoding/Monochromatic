...
async function UnlockRed() {
    ...
    dialogBox.show();
    BGM();
    await dialogBoxFunction("The blood....");
    await dialogBoxFunction("I feel crazy!!!!");
    await dialogBoxFunction("Maybe the room had hidden some secret in different color...");
    isControlingCharacter = 1;
    await dialogBoxFunction("Click the button to change the color of the room");
    dialogBox.hide();
}
...
async function livingroom_sovaDialogBox() {
    if (UnlockedStatus == 0) UnlockRed();
        ...
        if (ColorStatus == 1) {
            await dialogBoxFunction("There is a corpse and large quantity of bloods flowing...");
        }

        if (ColorStatus == 2) {
            await dialogBoxFunction("We used to sit here and watch TV together, I love them");
        }

        if (ColorStatus == 0) {
            if (haveTV == 0) {
                await dialogBoxFunction("You received a TV remote controler...");
                haveTV = 1;
                controlerState = 1;
            } else {
                await dialogBoxFunction("TV remote controler... what should I watch?");
            }
        }
...
async function livingroom_bigTableDialogBox() {
    ...
    if (ColorStatus == 2) {
        await dialogBoxFunction("I love to play video games with my Dad!");
    }

    if (ColorStatus == 1) {
        await dialogBoxFunction("I hate my sister, she steal the love from Mom and Dad!");
    }

    if (ColorStatus == 0) {
        await dialogBoxFunction("We used to eat meals together at this table...");
    }
...
async function livingroom_flowerDialogBox() {
    ...
    if (ColorStatus == 2) {
        await dialogBoxFunction("This is my mom's favorite plants!");
    }

    if (ColorStatus == 1) {
        await dialogBoxFunction("Leaves seems differents from I remembered...");
    }

    if (ColorStatus == 0) {
        await dialogBoxFunction("Three plants, like the color, R, Y ,B");
    }
...
async function livingroom_photoDialogBox() {
    ...
    if (ColorStatus == 2) {
        dialogBox.show();
        await dialogBoxFunction("I love my family....");
        await dialogBoxFunction("They treat me well and I love them...");
        dialogBox.hide();
    }
    if (ColorStatus == 1) {
        dialogBox.show();
        await dialogBoxFunction("I finally fulfill my dream, killed them...");
        await dialogBoxFunction("Wait.... What!? I killed them !?");
        dialogBox.hide();
    } else if (ColorStatus == 0) {
        dialogBox.show();
        await dialogBoxFunction("This is my family. However, it was a terrible memory.");
        await dialogBoxFunction("When I heard dad said 'Let us take the picture'...");
        await dialogBoxFunction("They took the picture without waiting me....");
        dialogBox.hide();
    }
...
async function livingroom_door2outsideDialogBox() {
    ...
    if (ColorStatus == 1) {
        await dialogBoxFunction("The door is locked so no one will discover people's death...");
    }
