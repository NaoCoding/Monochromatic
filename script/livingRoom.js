...
async function livingroom_sovaDialogBox() {
  if (UnlockedStatus == 0) UnlockRed();
    ...
    if (ColorStatus == 1) {
      dialogBoxFunction(
        "There is a corpse and large quantity of bloods flowing..."
      );
      await delay(3000);
    }
...
async function living_roomSovaAnimation() {
  ...
  var q = setInterval(() => {
    now = now == 1 ? 2 : 1;
    if (ColorStatus == 1) {
      livingroom_sova.attribute(
        "src",
        "image/red/livingroom_sova" + now.toString() + ".png"
      );
    } else {
      livingroom_sova.attribute(
        "src",
        "image/livingroom_sova" + now.toString() + ".png"
      );
    }
  }, 100);
}
...
async function livingroom_photoDialogBox() {
  ...
  if (ColorStatus == 1)
    photoLabel.attribute("src", "image/red/livingroom_photo.png");
  else if (ColorStatus == 0)
    photoLabel.attribute("src", "image/livingroom_photo.png");
  var q = setInterval(() => {
    if (photoAppear == 0) {
      clearInterval(q);
      photoLabel.hide();
...
async function livingroom_door2outsideDialogBox() {
  ...
  if (ColorStatus == 1) {
    dialogBoxFunction(
      "The door is locked so no one will discover people's death..."
    );
    await delay(3250);
  }
...
function start_SceneLivingRoom() {
  ...
  livingroom_bigTable.style(
    "position:absolute;top:35%;left:53%;width:17%;height:25%;"
  );
  livingroom_bigTable.attribute("id", "livingroom_bigTable");
  livingroom_bigTable.show();

  livingroom_door2bed = createImg("image/livingroom_door2bed.png", "bed");
  livingroom_door2bed.style(
    "position:absolute;top:59%;left:15%;width:3%;height:20%;"
  );
  livingroom_door2bed.attribute("id", "livingroom_door2bed");
  livingroom_door2bed.show();

  livingroom_door2outside = createImg(
    "image/livingroom_door2outside.png",
    "bed"
  );
  livingroom_door2outside.style(
    "position:absolute;top:81%;left:69%;width:11%;height:4%;"
  );
  ...
  livingroom_tv.style(
    "position:absolute;top:30%;left:15%;width:4%;height:18%;"
  );
