async function passwordDoorDialogBox() {
    ...
    keypad.style("display: grid; grid-template-columns: repeat(3, 1fr); grid-gap: 10px; margin-top: 20px;");
    ...
    async function checkPassword() {
        if (currentInput === "0517") {
            ...
            dialogBox.show();
            await dialogBoxFunction("Unlocked!");
            dialogBox.hide();
            doorWindow.hide();
        } else {
            dialogBox.show();
            await dialogBoxFunction("Incorrect password, please try again... Hint: Birthday");
            dialogBox.hide();
            currentInput = "";
            updateDisplay();
        }
...
async function passwordDoorForBottomDialogBox() {
    let doorWindow = createDiv("");
    doorWindow.style("position", "absolute");
    doorWindow.style("top: 10%; left: 25%");
    doorWindow.style("width: 50%; height: 80%");
